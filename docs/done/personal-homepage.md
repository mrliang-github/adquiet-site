...
- `tools/` 集中列出对外产品、公开工具和需要登录的内部工作台。
- `about/` 用可核验的个人介绍说明背景和联系方式。

不做：账户、评论、数据库、订阅、CMS、全站暗色模式切换、实时 X API 同步或营销数据采集。
...
- 首页使用黑灰网格、红色点缀和紧凑 Bento 卡片；阅读和内容页维持纸张色背景、深墨色正文，避免让首页的深色样式外溢。
...
- 2026-09-20：首页按参考站纠偏为四列八行的紧凑布局；地球缩为底部交互卡片，未引入头像、大地球或全站暗色主题。
