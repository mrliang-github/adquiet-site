...
<head>
  ...
  <meta name="theme-color" content="#0B1118">${robotsMeta}
...
function englishCard(lesson) {
  ...
  return `<p class="card-meta">${escapeHtml(lesson.scenario)} · ${escapeHtml(lesson.profession)}</p><h2><a href="/english/${lesson.slug}/">${escapeHtml(lesson.title)}</a></h2><p>${lesson.durationMinutes} 分钟 · ${escapeHtml(lesson.level)}</p>${previewMarker(lesson)}${lessonCallToAction(lesson)}`;
}

function globeMarkup() {
  return `<section class="bento-card bento-card--globe" data-globe aria-labelledby="globe-title">
  <div class="card-heading"><p class="card-kicker">ROTATING GLOBE</p><h2 id="globe-title">别只盯着一个窗口。</h2></div>
  <p class="globe-intro">地球会在进入视口后加载。可横向拖动、暂停，或用方向按钮调整视角。</p>
  <div class="globe-stage"><svg class="globe-canvas" data-globe-canvas viewBox="0 0 480 480" role="img" aria-label="可交互的世界地球" tabindex="0"><circle cx="240" cy="240" r="202" class="globe-sphere"></circle><path class="globe-grid" d="M38 240h404M240 38v404M88 108c71 38 233 38 304 0M88 372c71-38 233-38 304 0"></path><path class="globe-outline" d="M240 38a202 202 0 1 0 0 404a202 202 0 1 0 0-404"></path></svg><p class="globe-status" data-globe-status aria-live="polite">静态地球轮廓已就绪。</p></div>
  <div class="globe-controls" aria-label="地球控制"><button type="button" data-globe-left aria-label="向左旋转地球">←</button><button type="button" data-globe-pause aria-pressed="false">暂停旋转</button><button type="button" data-globe-right aria-label="向右旋转地球">→</button><button type="button" data-globe-reset>重置</button><button type="button" data-globe-retry hidden>重试加载</button></div>
</section>`;
}
...
function homePage({ articles, dailyReports, englishLessons }) {
  ...
  if (latestLesson) scripts.unshift("/site-assets/lesson-progress.js");
  return documentPage({
    pathname: "/",
    title: "个人产品与学习空间",
    ...
    body: `<main id="content" class="site-main bento-main"><div class="site-shell"><div class="bento-grid">
      <section class="bento-card bento-card--identity" aria-labelledby="home-title"><p class="card-kicker">LIANGXIAO / PERSONAL SPACE</p><h1 id="home-title">良逍。<br>产品经理，也在用 AI 做自己的产品。</h1><p class="identity-copy">这里放作品、实践记录，以及每天还在继续验证的发现和学习。</p><div class="card-actions"><a class="button-link" href="/writing/">读文章</a><a class="button-link button-link--quiet" href="/about/">认识我</a></div></section>
      <section class="bento-card bento-card--daily" aria-labelledby="daily-card-title"><p class="card-kicker">WIND NOTES / 风向标日报</p><div id="daily-card-title">${dailyCard(latestDaily)}</div></section>
      <section class="bento-card bento-card--english" aria-labelledby="english-card-title"><p class="card-kicker">DAILY ENGLISH / 英语练习</p><div id="english-card-title">${englishCard(latestLesson)}</div></section>
      <section class="bento-card bento-card--tools" aria-labelledby="tools-card-title"><div class="card-heading"><p class="card-kicker">TOOLS</p><h2 id="tools-card-title">正在用，也在维护。</h2></div><div class="bento-tool-list">${tools.slice(0, 3).map(compactTool).join("")}</div><a class="card-link" href="/tools/">查看全部工具</a></section>
      <section class="bento-card bento-card--writing" aria-labelledby="writing-card-title"><div class="card-heading"><p class="card-kicker">WRITING</p><h2 id="writing-card-title">最近写下来的</h2></div><div class="bento-list">${articles.slice(0, 2).map(compactArticle).join("")}</div><a class="card-link" href="/writing/">浏览文章</a></section>
      ${globeMarkup()}
      <section class="bento-card bento-card--now" aria-labelledby="now-card-title"><div class="card-heading"><p class="card-kicker">NOW</p><h2 id="now-card-title">最近在做</h2></div><div class="now-stack">${buildNotes.slice(0, 2).map((note) => `<article><time datetime="${note.date}">${dateLabel(note.date)} / ${escapeHtml(note.label)}</time><h3><a href="/writing/${note.articleSlug}/">${escapeHtml(note.title)}</a></h3></article>`).join("")}</div><a class="card-link" href="/building/">看构建记录</a></section>
      <section class="bento-card bento-card--contact" aria-labelledby="contact-card-title"><p class="card-kicker">STAY IN TOUCH</p><h2 id="contact-card-title">想聊 AI 工具、独立开发或出海产品？</h2><p>可以从公开文章、GitHub 项目或 X 上的短更新开始。</p><div class="card-actions"><a class="button-link button-link--quiet" href="${escapeAttribute(site.githubProfile)}" target="_blank" rel="noopener noreferrer">GitHub</a><a class="button-link button-link--quiet" href="${escapeAttribute(site.xProfile)}" target="_blank" rel="noopener noreferrer">X</a></div></section>
    </div></div></main>`
  });
}
...
async function copyStaticAssetsForPreview() {
  ...
  await cp(resolve("site-assets"), join(outputDirectory, "site-assets"), { recursive: true, force: true });
}
